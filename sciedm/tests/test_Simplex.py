"""Simplex tests against pyEDM validation files."""
...
def XXtestXX_simplex_valid_lib(): # <-- validLib not in sciedm Do not run
...
def test_simplex_disjoint_pred_nan():
    ...
    data = load_dataset('Lorenz5D').copy()
...
def test_simplex_datetime_index():
    ...
    data = load_dataset('SumFlow')
    kwargs = SimplexArgs.copy()
    kwargs.update( dict( columns = 'SumFlow',
                         target  = 'SumFlow',
                         lib     = [1, 800],
                         pred    = [801, 1001],
                         E       = 3,
...
def test_simplex_knn1_neighbors():
    ...
    assert array_equal( smpx.knn_neighbors_, knnValid )
...
def test_simplex_exclusion_radius_knn():
    ...
    assert array_equal( smpx.knn_neighbors_[:, 0], knnValid )
