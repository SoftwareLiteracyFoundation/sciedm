...
class CCM(TransformerMixin, BaseEstimator):
    ...
    Internal attributes (assigned after corresponding stage)
    ---------------------------------------------------------
    After ``Validate``:
    ...
    After ``Project``:
        ...
        libMeans_ — DataFrame returned by the most recent Project call.
    ...
    def get_feature_names_out(self, input_features=None):
        ...
        col_tgt = f"{self._columns}:{self._target}"
        tgt_col = f"{self._target}:{self._columns}"

        if self.includeData:
            arr = array(["LibSize",col_tgt,tgt_col], dtype=object,)            
        else:
            arr = array(
                [
                    "LibSize",
    ...
    def transform(self, X):
        ...
        X = validate_data(
            ...
            skip_check_array=True, reset=False
        )
    ...
    def Validate(self):
        ...
        if self.Data_ is None :
            ...
            if not isinstance( self.Data_, DataFrame ) :
        ...
        for column in self.columns :
            if not column in self.Data_.columns :
        ...
        for target in self.target :
            if not target in self.Data_.columns :
        ...
        if len( self.validLib ):
            if len( self.validLib ) != self.Data_.shape[0] :
                raise ValueError(f'Validate() {self.name}:' +\
        ...
        if self.libSizes[-1] > self.Data_.shape[0] :
            msg = f'{self.name} Validate(): ' +\
                  f'Maximum libSize {self.libSizes[-1]}'    +\
                  f' exceeds data size {self.Data_.shape[0]}.'
            raise ValueError( msg )
        ...
        self.tgt_vec = np.asarray(self.Data_[self.target[0]], dtype=np.float64)
        self.col_pred_vec = np.asarray(
            self.Data_[self.columns[0]], dtype=np.float64
        )
        ...
        self._col_vectors = {
            name: np.asarray(self.Data_[name], dtype=np.float64)
    ...
    def Project(self,
        ...
        Returns
        -------
        self.libMeans_ : pandas.DataFrame
        """
        U = CCM._UNSET
        libSizes    = self.libSizes     if libSizes     is U else libSizes
        sample      = self.sample       if sample       is U else sample
        random_state= self.random_state if random_state is U else random_state
        ...
        root_seq  = np.random.SeedSequence(random_state)
